    ...
    def __combine_children_to_tree(self, ui_data, start_element):
        ...
        if 'children-id' in element_cp:
            ...
            for c_id in element_cp['children-id']:
                element_cp['children'].append(self.__combine_children_to_tree(ui_data=ui_data, start_element=ui_data.elements[c_id]))
            self.__select_ele_attr(element_cp, ['scrollable', 'id', 'resource-id', 'class', 'clickable', 'children', 'description'])
        else:
