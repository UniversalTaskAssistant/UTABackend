...
class TaskDeclarator:
    def __init__(self, model_manager):
        ...
        self.base_prompt_clarify = 'Assess the user task "{task}" to determine if it is sufficiently clear for execution ' \
        ...
        self.base_prompt_decompose = 'Analyze the user task "{task}" to determine if it comprises multiple, distinct sub-tasks. ' \
        ...
        self.base_prompt_classify = 'Classify the given user task "{task}" into one of three categories for smartphone usage: ' \
    ...
    def clarify_task(self, task, user_message=None, printlog=False):
        ...
        try:
            ...
            if len(task.conversation_clarification) == 1:
                task.conversation_clarification.append({'role': 'user', 'content': self.base_prompt_clarify.format(task=task.task_description)})
            # add user feedback
            if user_message:
