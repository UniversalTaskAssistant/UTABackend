...
class TaskDeclarator:
    def __init__(self, model_manager):
        ...
        self.base_prompt_clarify = 'Assess the user task "{task}" to determine if it is sufficiently clear for ' \
                                   'execution on a smartphone. Given that seniors often provide vague or incomplete ' \
                                   'task descriptions, identify the most crucial piece of missing information. Ask a ' \
                                   'single, focused question that is most likely to clarify the task effectively.' \
        ...
        self.base_prompt_decompose = 'Analyze the user task "{task}" to determine if it comprises multiple, distinct ' \
                                     'sub-tasks. Complex tasks often consist of several steps that need to be ' \
                                     'executed separately. ' \
                                     ...
                                     'Identify if the given task requires decomposition and if so, break it down into ' \
                                     'its constituent sub-tasks. Provide your analysis in JSON format, including: ' \
        ...
        self.base_prompt_classify = 'Classify the given user task "{task}" into one of three categories for ' \
                                    'smartphone usage: ' \
                                    '1. General Inquiry: This category includes tasks that are general questions not ' \
                                    'related to specific system or app functions. They can typically be answered ' \
                                    'through internet searches. Example: "What is the weather today?" ' \
                                    '2. System Function: These are tasks that involve system-level functions of the ' \
                                    'smartphone. Examples include adjusting settings or using built-in features ' \
                                    'like alarms. ' \
                                    ...
                                    '1. "Task Type" indicating the category, and 2. "Explanation" providing a brief ' \
                                    'rationale for your classification. ' \
                                    'Example: {{"Task Type": "General Inquiry", "Explanation": "This task is a ' \
                                    'general query that can be resolved through an internet search, without the need ' \
                                    'for system-level access or specific apps."}}'
    ...
    def clarify_task(self, task, user_message=None, printlog=False):
        ...
        try:
            ...
            if len(task.conversation_clarification) == 1:
                task.conversation_clarification.append({'role': 'user', 'content': self.base_prompt_clarify.
    ...
    def decompose_task(self, task, printlog=False):
        ...
        try:
            conversation = [{'role': 'system', 'content': SYSTEM_PROMPT},
                            {'role': 'user', 'content': self.base_prompt_decompose.format(task=task.task_description)}]
    ...
    def classify_task(self, task, printlog=False):
        ...
        try:
            conversation = [{'role': 'system', 'content': SYSTEM_PROMPT},
                            {'role': 'user', 'content': self.base_prompt_classify.format(task=task.task_description)}]
            resp = self.__model_manager.send_fm_conversation(conversation=conversation, printlog=printlog)
